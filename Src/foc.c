...
{
	const float StandardizationCoefficient = SQRT3 * TIM8_ARR / 24.f;
	float U1 = 0;
	float U2 = 0;
	float U3 = 0;
	float Tx = 0;
...
// --- 用最终 CCR 反推实际占空比（避免整除/避免用TA TB TC） ---
// da = (float)CCR_PHASE_A / (float)TIM8_ARR;
// db = (float)CCR_PHASE_B / (float)TIM8_ARR;
// dc = (float)CCR_PHASE_C / (float)TIM8_ARR;



//// --- pole voltage 去公共模式 -> 相电压(线-中性) ---
// d0 = (da + db + dc) / 3.0f;
// va = (da - d0) * 24;
// vb = (db - d0) * 24;
//// vc = (dc - d0) * 24;

//// --- Clarke(与你 arm_clarke_f32 一致的定义) ---
//CoordTrans.true_VolAlpha = va;
//CoordTrans.true_VolBeta  = 0.57735026919f * va +  1.15470053838f * vb;
}
